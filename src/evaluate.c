...
const int PawnDoubled  = S(-13,-55);
const int PawnDoubled2 = S(-10,-26);
const int PawnIsolated = S( -6,-16);
const int PawnSupport  = S( 22, 17);
const int PawnThreat   = S( 79, 36);
const int PushThreat   = S( 25,  6);
const int PawnOpen     = S(-14,-22);
const int BishopPair   = S( 33,110);
const int KingAtkPawn  = S(  0, 35);
const int OpenForward  = S( 31, 29);
const int SemiForward  = S( 17, 17);
const int NBBehindPawn = S(  9, 32);

// Passed pawn
const int PawnPassed[RANK_NB] = {
    S(  0,  0), S(-17, 24), S(-21, 38), S(-64,110),
    S(-10,132), S(107,148), S(357,182), S(  0,  0)
};
const int PassedDefended[RANK_NB] = {
    S(  0,  0), S(  0,  0), S(  2,-13), S(  2, -9),
    S( -2, 37), S( 29,113), S(137,121), S(  0,  0)
};
const int PassedBlocked[4] = {
    S(  0, -5), S( -5,  4), S(-12, -8), S(-71,-37)
};
const int PassedFreeAdv[4] = {
    S( -5, 25), S(-15, 61), S(-33,194), S(-123,316)
};
const int PassedDistUs[4] = {
    S( 16,-32), S( 13,-42), S( -6,-45), S(-26,-32)
};
const int PassedDistThem = S( -5, 21);
const int PassedRookBack = S( 22, 46);
const int PassedSquare   = S(-1668,452);

// Pawn phalanx
const int PawnPhalanx[RANK_NB] = {
    S(  0,  0), S(  9, -2), S( 20, 15), S( 35, 38),
    S( 70,126), S(189,414), S(-154,757), S(  0,  0)
};

// Threats
const int ThreatByMinor[8] = {
    S(  0,  0), S(  0,  0), S( 30, 40), S( 39, 46),
    S( 72, 12), S( 58, -4), S(  0,  0), S(  0,  0)
};
const int ThreatByRook[8] = {
    S(  0,  0), S(  0,  0), S( 26, 46), S( 35, 47),
    S(-10, 46), S( 87,-49), S(  0,  0), S(  0,  0)
};

// KingLineDanger
const int KingLineDanger[28] = {
    S(  0,  0), S(  0,  0), S( 35,-32), S( 15, 17),
    S(-14, 34), S(-24, 30), S(-28, 29), S(-37, 39),
    S(-49, 42), S(-68, 43), S(-71, 44), S(-85, 48),
    S(-91, 48), S(-104, 48), S(-117, 51), S(-116, 47),
    S(-122, 44), S(-133, 41), S(-131, 36), S(-134, 29),
    S(-142, 27), S(-157, 22), S(-155, 19), S(-165,  1),
    S(-146,-19), S(-134,-30), S(-130,-32), S(-135,-34)
};

// Mobility [pt-2][mobility]
const int Mobility[4][28] = {
    // Knight (0-8)
    { S(-74,-78), S(-37,  3), S(-14, 65), S( -1, 83), S( 11, 97), S( 20,114), S( 30,115), S( 42,115),
      S( 53, 94) },
    // Bishop (0-13)
    { S(-63,-35), S(-31,-14), S(-14, 33), S( -4, 61), S(  8, 73), S( 20, 91), S( 25,106), S( 32,111),
      S( 33,119), S( 37,120), S( 46,115), S( 64,106), S( 60,111), S( 74, 87) },
    // Rook (0-14)
    { S(  8,-275), S(-19, 43), S( -4, 93), S(  5, 95), S(  2,126), S(  7,135), S(  5,150), S( 13,151),
      S( 15,158), S( 20,162), S( 25,170), S( 22,180), S( 23,188), S( 32,193), S( 38,190) },
    // Queen (0-27)
    { S(-354,-244), S(-218,-327), S(-78,-71), S(-39,  0), S(-43,158), S(-32,180), S(-15,195), S(-13,223),
      S( -9,255), S( -3,257), S(  0,267), S(  3,276), S(  9,277), S(  9,288), S( 11,298), S( 14,300),
      S( 12,311), S( 13,319), S(  6,338), S(  4,352), S(  9,351), S(  8,358), S( 25,362), S( 23,363),
      S( 59,350), S(101,293), S(244,202), S(101,280) }
};
